...
export default function VirtualPiano({
  ...
  const noteToFrequency = (note: string, octave: number): number => {
    ...
    const semitone = NOTE_NAMES.indexOf(note) + (octave - A4_OCTAVE) * 12;
        ...
        <div className="relative" style={{ width: WHITE_KEY_WIDTH * whiteKeys.length }}>
          {whiteKeys.map((key, index) => {
            ...
            return (
              <div
                key={noteId}
                className={`absolute top-0 w-10 h-40 rounded-b-lg cursor-pointer transition-all ${
                  isActive
        ...
        >
          {blackKeys.map((key) => {
            ...
            return (
              <div
                key={noteId}
                className={`absolute w-6 h-24 rounded-b-lg cursor-pointer transition-all z-10 ${
                  isActive
